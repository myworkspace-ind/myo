...
import Cropper from "../assets/cropperjs/dist/cropper.esm.js?version=bcff1744";
import { loadProperties } from "../sakai-i18n.js?version=bcff1744";
import { getUserId } from "../sakai-portal-utils.js?version=bcff1744";
export class SuiPictureChanger extends SakaiElement {
  ...
  _refreshProfileImageTagsAndHideDialog() {
    ...
    document.querySelectorAll(".sakai-accountProfileImage").forEach(pic => pic.setAttribute("src", imageUrl));
  ...
  _remove() {
    ...
    fetch(url, {
      ...
      method: "POST",
      body: JSON.stringify({
        sakai_csrf_token: this.csrfToken
      })
    }).then(r => {
      if (r.ok) {
