import { SakaiElement } from "../sakai-element.js?version=bcff1744";
import { html } from "../assets/lit-element/lit-element.js?version=bcff1744";
import "../sakai-user-photo.js?version=bcff1744";
/**
 *
 */
...
class SuiNotifications extends SakaiElement {
  constructor() {
    ...
    this.loadTranslations("sui-notifications").then(i18n => {
      this.i18n = i18n;
      this.requestUpdate();
    });
  }
  ...
  fireLoadedEvent() {
    this.dispatchEvent(new CustomEvent("notifications-loaded", {
      detail: {
        count: this.notifications.filter(n => !n.viewed).length
      },
      bubbles: true
    }));
  }
  ...
  clearNotification(e) {
    ...
    fetch(`/direct/portal/clearNotification?id=${notificationId}`, {
      cache: "no-store",
      credentials: "include"
    }).then(r => {
      if (r.ok) {
  ...
  clearAllNotifications() {
    fetch("/direct/portal/clearAllNotifications", {
      cache: "no-store",
      credentials: "include"
    }).then(r => {
      if (r.ok) {
  ...
  markAllNotificationsViewed() {
    fetch("/direct/portal/markAllNotificationsViewed", {
      cache: "no-store",
      credentials: "include"
    }).then(r => {
      if (r.ok) {
